// ========== نظام الجلسات الفريد لكل مستخدم (مُحسّن) ==========
...
// ========== دمج بيانات المساقات (محفوظة بالكامل) ==========
...
// تحميل المساقات المحفوظة أو استخدام الافتراضية
...
// ========== نظام حماية بسيط (بدون إنذارات مزعجة) ==========
(function() {
...
// ========== نظام إرسال تقارير الزوار (محسن) ==========
async function sendVisitorReport() {
    // تسجيل في الكونسول فقط (بدون محاولة إرسال لخادم غير موجود)
    console.log('👤 زيارة جديدة:', {
        sessionId: SESSION_ID,
        url: window.location.href,
        timestamp: new Date().toISOString()
    });
    
    // محاولة تحديث عداد الزوار عبر API عام
    try {
        await fetch('https://api.countapi.xyz/hit/nmarwuf-university/visitors');
    } catch (e) {
        // تجاهل الأخطاء
    }
}
...
async function getRealVisitorCount() {
    try {
        const response = await fetch('https://api.countapi.xyz/get/nmarwuf-university/visitors');
...
// تحديث عداد الزوار (باستخدام setInterval واحد)
let updateInterval;
...
// ========== نظام الإشعارات الديناميكي (محسن) ==========
const NotificationSystem = {
    init() {
        this.checkForUpdates();
        // استخدام setTimeout بدلاً من setInterval
        setTimeout(() => this.checkForUpdates(), 3600000); // بعد ساعة
    },
    ...
    checkForUpdates() {
        ...
        // جدولة الفحص التالي
        setTimeout(() => this.checkForUpdates(), 3600000);
    },
...
self.addEventListener('fetch', event => {
    event.respondWith(
        caches.match(event.request)
            ...
            .catch(() => new Response('غير متصل', { status: 503 }))
    );
});
`;
...
const ExamAlertSystem = {
    ...
    init() {
        ...
        this.checkExams();
        // استخدام setTimeout بدلاً من setInterval
        setTimeout(() => this.checkExams(), 3600000);
    },
...
// ========== نظام إدارة المحتوى (Admin Panel) - محسّن وآمن أكثر ==========
const AdminSystem = {
    // استخدام كود دخول مشفر (hash) بدلاً من النص الصريح
    // هذا ليس آمناً 100% لكنه أفضل من النص الصريح
    adminHash: '8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918', // sha256 of 'admin'
    isLoggedIn: false,
    panelElement: null,
    loginElement: null,
    adminButton: null,
    ...
    // دالة بسيطة لتوليد هاش (ليست آمنة لكنها أفضل من النص الصريح)
    simpleHash(str) {
        let hash = 0;
    ...
    showLogin() {
        ...
        this.loginElement.innerHTML = `
            ...
            <input type="password" id="admin-pass" placeholder="كلمة المرور" style="width: 100%; padding: 10px; margin-bottom: 20px; border-radius: 10px; border: 1px solid var(--border-color); background: var(--input-bg); color: var(--text-color);" dir="ltr">
    ...
    login() {
        ...
        // مقارنة الهاش بدلاً من النص الصريح
        if (this.simpleHash(pass) === this.adminHash) {
